...
app.use((req, res, next) => {
  const cspReportOnly = [
    "default-src 'self'",
    `script-src 'self' 'nonce-${res.locals.cspNonce}' https://cdn.jsdelivr.net https://unpkg.com`,
...
// Main entry: redirect root URL to Nortek landing page
app.get('/', (_, res) => res.redirect('/nortek.html'));
    ...
    attachments: [
      {
        ...
        path: path.join(__dirname, "public/images/nortek_white.png"), // adjust path
        cid: "norteklogo" 
      }
    ]
  });
      ...
      try {
        await transporter.sendMail({
          ...
          attachments: [
            {
              filename: "nortek_white.png",
              path: path.join(__dirname, "public/images/nortek_white.png"),
              cid: "norteklogo"
            }
          ]
        });
